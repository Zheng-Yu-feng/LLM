<template>
    <div class="chat-container">
      <!-- <div class="back-home-button" @click="goHome">
        <el-icon size="26px">
          <i-ph-arrow-left />
        </el-icon>
        返回首页
      ...
      <div class="model-selector">
        <el-select v-model="selectedModel" placeholder="选择语言模型" size="small" @change="onModelChange">
          <el-option label="GPT-3" value="gpt3"></el-option>
            ...
            <template v-for="(item, index) in chatList" :key="index">
              <div class="chat-list__item" v-if="item.role !== 'system'">
                <div class="message-card animate__animated animate__fadeInUp"
                  ...
                  <!-- ✅ 显示生成时间（仅助手消息显示） -->
                  <div class="gen-time" v-if="item.role === 'assistant' && item.gen_time">
                    生成时间：{{ item.gen_time }}
                  </div>
    ...
    <div class="chat-enter animate__animated animate__fadeInUp">
      ...
      <div class="enter-wrapper">
        ...
        <el-input type="textarea" placeholder="有问题尽管问我..." resize="none" maxlength="2000" enterkeyhint="send"
          autocorrect="off" show-word-limit :autosize="autosizeConfig" v-model="userInput" @focus="focusHandle"
...
function goHome() {
  nextTick(() => {
    router.push('/home');  // 确保在下一轮 DOM 更新后进行路由跳转
  });
}
...
const chatList = computed(() => store.currentMessages)
const autosizeConfig = ref({ minRows: 1, maxRows: 8 })
...
//   store.addMessage = res.data.history;
// });
  ...
  try {
    const res = await fetch('/api/chat', {
      ...
      body: JSON.stringify({
        ...
        model_type: modelStore.currentModelSource,
        currentMessage
      }),
    })
    ...
    const elapsed = ((endTime - startTime) / 1000).toFixed(2) + 's' // ✅ 转为秒，保留两位小数

    store.addMessage({
      role: 'assistant',
      content: data.response,
      gen_time: elapsed // ✅ 添加生成时间
    })
...
.message-card.is-right .gen-time {
  text-align: left;
}
